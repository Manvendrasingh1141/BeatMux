    ...
    <div className="bg-white/90 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col shrink-0">
      ...
      <div className="flex items-center justify-between px-4 border-b border-slate-100">
        <div className="flex items-center">
          ...
          {/* Add Music */}
          <label className="flex items-center gap-1 text-[12px] font-medium text-slate-400 hover:text-indigo-600 transition-colors py-3 cursor-pointer">
            <Plus className="w-3.5 h-3.5" />
            Add Music
            <input type="file" accept="audio/*" className="hidden" onChange={(e) => {
              if (e.target.files?.[0] && window.onAddMusic) {
                window.onAddMusic(e.target.files[0]);
              }
              e.target.value = '';
            }} />
          </label>
