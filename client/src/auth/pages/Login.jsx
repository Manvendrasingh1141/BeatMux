...
const Login = () => {
  ...
  const validate = (name, val) => {
    ...
    if (name === 'password') return val.trim() ? null : 'Password is required.';
    return null;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (code === 'INVALID_CREDENTIALS') setServerError('Incorrect email or password.');
      else if (code === 'ACCOUNT_DISABLED') setServerError('Your account has been disabled.');
      else if (code === 'RATE_LIMITED') setServerError('Too many login attempts. Please try again later.');
      else setServerError(err.response?.data?.message || 'Failed to sign in. Please try again.');
    } finally {
      ...
      <div className="w-full max-w-7xl mx-auto px-6 py-8 flex flex-col lg:flex-row items-center justify-between gap-12 relative z-10 flex-grow">
        ...
        <div className="w-full lg:w-[40%] max-w-md">
          <div className="bg-white rounded-[24px] shadow-xl shadow-indigo-900/5 border border-slate-100 overflow-hidden relative">
            ...
            <div className="p-10">
              <h3 className="text-2xl font-bold text-slate-900">Studio Sign In</h3>
              <p className="text-sm text-slate-500 mt-1 mb-8">Sign in and keep the beat going</p>

              <form onSubmit={handleSubmit} className="space-y-6" noValidate>
                <div>
                  <label className="block text-xs font-bold text-slate-700 tracking-wider mb-2">STUDIO EMAIL</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full pl-11 pr-4 py-3 rounded-xl bg-white border outline-none transition-all text-slate-800 text-sm ${
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 tracking-wider mb-2">PASSWORD</label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    ...
                    <input
                      ...
                      className={`w-full pl-11 pr-11 py-3 rounded-xl bg-white border outline-none transition-all text-slate-800 text-sm ${
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3.5 px-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold transition-all shadow-md shadow-indigo-600/20 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {loading ? 'Signing in...' : 'Open Sequencer Studio'}
              ...
              <div className="mt-6 text-center">
                <span className="text-xs text-slate-500">Don't have a studio key? </span>
                <Link to="/register" className="text-xs font-bold text-indigo-600 hover:text-indigo-800 underline underline-offset-2">
                  Register
                </Link>
              </div>
            </div>
